...
private:
   ...
   *
   * @param prefix Filename prefix to use for pcap files.
   * @param c2c Ptr to the c2c interface on which you want to enable tracing.
   * @param interface Interface ID on the c2c on which you want to enable tracing.
   * @param explicitFilename Treat the prefix as an explicit filename if true
   */
...
//   * \internal
//   */
//  static void AsciiDropEventc2c (Ptr<AsciiWriter> writer, std::string path,  //TODO da commentare???
...
//  /*iTetris INTECS*/
   ...
   * \internal
   */
  bool PcapHooked (Ptr<c2c> c2cVar);

  /**
   * \internal
   */
  bool AsciiHooked (Ptr<c2c> c2cVar);
  /* iTetris INTECS Azzarelli */
