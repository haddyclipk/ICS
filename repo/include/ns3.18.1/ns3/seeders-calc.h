...
class DerivativeInjection : public SeedersCalc
{
public:
	static TypeId GetTypeId (void);
	DerivativeInjection ();
	virtual ~DerivativeInjection ();
...
private:
	// a map where the key is the time and the value is the infection ratio
	std::map<double,double> infectionEvolutionMap;
	double d_windowSize;
	double clipping;
