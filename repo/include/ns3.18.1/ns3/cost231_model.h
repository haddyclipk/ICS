...
#ifndef COST231PROPAGATIONMODEL_H_
#define COST231PROPAGATIONMODEL_H_
 ...
 * Base station Antenna Hcisteight: 30m to 200m
 * Link Distance:up to 20 km
 *
 */

class Cost231Propagation : public Object
{
 ...
 */
 double
 GetLoss (Ptr<MobilityModel> a, Ptr<MobilityModel> b) const;
 void
 SetBSAntennaHeight (double height);
 void
 SetSSAntennaHeight (double height);
 void
 ...
 double
 GetBSAntennaHeight (void) const;
 double
 GetSSAntennaHeight (void) const;
 Environment
 GetEnvironment (void) const;
 double
 GetMinDistance (void) const;
 double
 GetLambda (void) const;
 void
 SetLambda (double frequency, double speed);
 double
 GetShadowing (void);
 void
 SetShadowing (double shadowing);
 /**
 * \param d the distnace between the sender and the receiver
 * \returns the bulk path loss
 */
 double
 cost231_formula (double d); // the formula that calculates the bulk path loss
private:
