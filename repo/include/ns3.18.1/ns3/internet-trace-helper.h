...
/* iTetris INTECS Azzarelli */
#include "ns3/c2c-interface-container.h"
/* iTetris INTECS Azzarelli */
#include "ns3/ipv4.h"
#include "ns3/ipv6.h"
...
public:
   ...
   * if it is non-null.  If the OutputStreamWrapper is null, the implementation
   * is expected to use a provided prefix to construct a new file name for
   * each net device using the rules described in EnablePcapIpv4the class overview.
   *
   ...
   *
   * @param stream An OutputStreamWrapper representing an existing file to use
   *               when writing trace data.EnablePcapIpv4
   * @param ipv6 Ptr<Ipv6> on which you want to enable tracing.
   * @param interface The interface on which you want to enable tracing.
   */
...
/**
 * @brief Base class providing common user-level pcap operations for helpers
 * representing C2c protocols .
 */
class PcapHelperForC2c
{
public:
  /**
   * @brief Construct a PcapHelperForC2c.
   */
  PcapHelperForC2c () {}

  /**
   * @brief Destroy a PcapHelperForc2c.
   */
  virtual ~PcapHelperForC2c () {}

  /**
   * @brief Enable pcap output the indicated c2c and interface pair.
   * @internal
   *
   * @param prefix Filename prefix to use for pcap files.
   * @param c2cVar Ptr<c2c> on which you want to enable tracing.
   * @param interface Interface on c2c on which you want to enable tracing.
   * @param explicitFilename Treat the prefix as an explicit filename if true
   */
  virtual void EnablePcapC2cInternal (std::string prefix,
                                       Ptr<c2c> c2cVar,
                                       uint32_t interface,
                                       bool explicitFilename) = 0;
   ...
   * @param explicitFilename Treat the prefix as an explicit filename if true.
   */
  void EnablePcapC2c (std::string prefix, Ptr<c2c> c2cVar, uint32_t interface, bool explicitFilename = false);

  /**
   * @brief Enable pcap output the indicated c2c and interface pair using a
   * Ptr<c2c> previously named using the ns-3 object name service.
   *
   * @param prefix filename prefix to use for pcap files.
   * @param c2cName Name of the Ptr<c2c> on which you want to enable tracing.
   * @param interface Interface on c2c on which you want to enable tracing.
   * @param explicitFilename Treat the prefix as an explicit filename if true.
   */
  void EnablePcapC2c (std::string prefix, std::string c2cName, uint32_t interface, bool explicitFilename = false);

  /**
   * @brief Enable pcap output on each c2c and interface pair in the container.
   *
   * @param prefix Filename prefix to use for pcap files.
   * @param c c2cInterfaceContainer of c2c and interface pairs
   */
  void EnablePcapC2c (std::string prefix, c2cInterfaceContainer c);

  /**
   * @brief Enable pcap output on all c2c and interface pairs existing in the
   * nodes provided in the container.
   *
   * \param prefix Filename prefix to use for pcap files.
   * \param n container of nodes.
   */
  void EnablePcapC2c (std::string prefix, NodeContainer n);

  /**
   * @brief Enable pcap output on the c2c and interface pair specified by a
   * global node-id (of a previously created node) and interface.  Since there
   * can be only one c2c aggregated to a node, the node-id unambiguously
   * determines the c2c.
   *
   * @param prefix Filename prefix to use for pcap files.
   * @param nodeid The node identifier/number of the node on which to enable tracing.
   ...
   * @param explicitFilename Treat the prefix as an explicit filename if true
   */
  void EnablePcapC2c (std::string prefix, uint32_t nodeid, uint32_t interface, bool explicitFilename);
   ...
   * @param prefix Filename prefix to use for pcap files.
   */
  void EnablePcapC2cAll (std::string prefix);
...
/**
 * @brief Base class providing common user-level ascii trace operations for
 * helpers representing c2c protocols .
 */
class AsciiTraceHelperForC2c
{
public:
  /**
   * @brief Construct an AsciiTraceHelperForC2c.
   */
  AsciiTraceHelperForC2c () {}

  /**
   * @brief Destroy an AsciiTraceHelperForC2c
   */
  virtual ~AsciiTraceHelperForC2c () {}

  /**
   * @brief Enable ascii trace output on the indicated c2c and interface pair.
   * @internal
   *
   * The implementation is expected to use a provided Ptr<OutputStreamWrapper>
   * if it is non-null.  If the OutputStreamWrapper is null, the implementation
   ...
   * each net device using the rules described in the class overview.
   *
   * If the prefix is provided, there will be one file per c2c and interface pair
   * created.  In this case, adding a trace context to the file would be pointless,
   * so the helper implementation is expected to TraceConnectWithoutContext.
   *
   * If the output stream object is provided, there may be many different c2c
   * and interface pairs writing to a single file.  In this case, the trace
   * context could be important, so the helper implementation is expected to
   * TraceConnect.
   *
   ...
   *               when writing trace data.
   * @param prefix Filename prefix to use for ascii trace files.
   * @param c2c Ptr<c2c> on which you want to enable tracing.
   * @param interface The interface on which you want to enable tracing.
   * @param explicitFilename Treat the prefix as an explicit filename if true.
   */
   ...
   * @param explicitFilename Treat the prefix as an explicit filename if true.
   */
  void EnableAsciiC2c (std::string prefix, Ptr<c2c> c2cVar, uint32_t interface, bool explicitFilename = false);
   ...
   * @param interface The interface on which you want to enable tracing.
   */
  void EnableAsciiC2c (Ptr<OutputStreamWrapper> stream, Ptr<c2c> c2cVar, uint32_t interface);

  /**
   * @brief Enable ascii trace output the indicated c2c and interface pair
   * using an c2c previously named using the ns-3 object name service.
   *
   * @param prefix filename prefix to use for ascii files.
   * @param c2cName The name of the c2c on which you want to enable tracing.
   * @param interface The interface on which you want to enable tracing.
   * @param explicitFilename Treat the prefix as an explicit filename if true.
   */
  void EnableAsciiC2c (std::string prefix, std::string c2cName, uint32_t interface, bool explicitFilename = false);
   ...
   * @param interface The interface on which you want to enable tracing.
   */
  void EnableAsciiC2c (Ptr<OutputStreamWrapper> stream, std::string c2cName, uint32_t interface);

  /**
   * @brief Enable ascii trace output on each c2c and interface pair in the
   * container
   *
   * @param prefix Filename prefix to use for ascii files.
   * @param c c2cInterfaceContainer of c2c and interface pairs on which to
   *          enable tracing.
   */
  void EnableAsciiC2c (std::string prefix, c2cInterfaceContainer c);
   ...
   *          enable tracing.
   */
  void EnableAsciiC2c (Ptr<OutputStreamWrapper> stream, c2cInterfaceContainer c);

  /**
   * @brief Enable ascii trace output on all c2c and interface pairs existing
   * in the nodes provided in the container.
   *
   * \param prefix Filename prefix to use for ascii files.
   * \param n container of nodes.
   */
  void EnableAsciiC2c (std::string prefix, NodeContainer n);
   ...
   * \param n container of nodes.
   */
  void EnableAsciiC2c (Ptr<OutputStreamWrapper> stream, NodeContainer n);
   ...
   * @param prefix Filename prefix to use for ascii files.
   */
  void EnableAsciiC2cAll (std::string prefix);
   ...
   *               when writing trace data.
   */
  void EnableAsciiC2cAll (Ptr<OutputStreamWrapper> stream);
   ...
   * @param explicitFilename Treat the prefix as an explicit filename if true
   */
  void EnableAsciiC2c (std::string prefix, uint32_t nodeid, uint32_t deviceid, bool explicitFilename);
   ...
   * @param explicitFilename Treat the prefix as an explicit filename if true
   */
  void EnableAsciiC2c (Ptr<OutputStreamWrapper> stream, uint32_t nodeid, uint32_t interface, bool explicitFilename);

private:
  /**
   * @internal Avoid code duplication.
   */
  void EnableAsciiC2cImpl (Ptr<OutputStreamWrapper> stream,
                            std::string prefix,
                            uint32_t nodeid,
                            uint32_t interface,
                            bool explicitFilename);
   ...
   * @internal Avoid code duplication.
   */
  void EnableAsciiC2cImpl (Ptr<OutputStreamWrapper> stream, std::string prefix, NodeContainer n);

  /**
   * @internal Avoid code duplication.
   */
  void EnableAsciiC2cImpl (Ptr<OutputStreamWrapper> stream, std::string prefix, c2cInterfaceContainer c);
