...
public:
    ...
    /// @brief Checks if within the application life_cycle, a specific subscription became required
    static tcpip::Storage CheckForRequiredSubscriptions_TT (int nodeId, int timestep);

    static bool SaveCurrentTLSstatus(std::string status,int timestemp);
...
private:
    ...
    static std::string tlsStatus;
    static int tlsTimestemp;
};
