...
Server::dispatchCommand()
{
    ...
    case CMD_APP_CLOSE: {
     ...
     case CMD_APP_RESULT_TRAFF_SIM: {
    	success = ReceiveAppResultTraffSim();
    	break;
    }
    default:
...
bool
Server::ReceiveAppResultTraffSim(){

    int timestemp = myInputStorage.readInt(); //timestemp
    int nodeId = myInputStorage.readInt();	/*nodeId*/
  
    int type = (int)myInputStorage.readUnsignedByte();
    ...
    while ( myInputStorage.valid_pos() ){
    	c = myInputStorage.readUnsignedByte();

    	i++;
    	value.push_back(c);
    }
	std::string tlsStatus(value.begin(), value.end() );
	
    ApplicationLogic::SaveCurrentTLSstatus(tlsStatus,timestemp);
      
 
    // create reply message  	
    writeStatusCmd(CMD_APP_RESULT_TRAFF_SIM, APP_RTYPE_OK, "");
...
Server::ReceiveSubscriptionCarsInZone()
{
    ...
    /*** MY CODE ***/
    //v.speed = myInputStorage.readFloat();  //AGGIUNGERE in tutte le apps
    //aggiungo la lane 
    //v.lane = myInputStorage.readString();  //AGGIUNGERE in tutte le apps
    /*** END OF MY CODE ***/
