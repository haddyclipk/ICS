...
std::string ApplicationLogic::tlsStatus="";
int ApplicationLogic::tlsTimestemp=0;
...
ApplicationLogic::CheckForRequiredSubscriptions (int nodeId, int timestep){
  ...
  for (vector<AppMessage>::iterator it_ = m_messages.begin(); it_ != m_messages.end() ; ++it_) {
       if (((*it_).status == kToBeScheduled) && ((*it_).senderId == nodeId)) { // we need to send an APP_MSG_SEND and it should be a subscription created by me
           ...
           std::cout << "SUB_APP_CMD_TRAFF_SIM ::  SET_VALUE_ ::(*it_).destinationId =" << (*it_).destinationId << std::endl;
			   ...
			   //mySubsStorage.writeFloat(8.0); //Set Maximum speed (8.0 in this case)
  ...
  cout << "-ApplicationLogic::CheckForRequiredSubscriptions()" << endl;
  return  mySubsStorage;
}

bool
ApplicationLogic::SaveCurrentTLSstatus(std::string status,int timestemp)
{
	tlsStatus = status;
	tlsTimestemp = timestemp;
	return true;
}
   ...
   if (timestep >= m_appStartTimeStep && senderId == 5000) {  // restrict the command to the RSU only (ID RSU is 5000)
        ...
        for (vector<Vehicle>::const_iterator it = m_vehiclesInArea.begin() ; it != m_vehiclesInArea.end() ; it++) {
           AppMessage message;
            ...
            message.payloadValue = (*it).speed; // we make the vehicle STOP !! message.payloadValue = 0
            ...
            //message.payloadValue = tlsStatus;
