...
std::vector<int> posElementsToBeRemoved;
AppMessage tmp;
for (unsigned i=0; i< m_messages.size(); ++i){
	tmp=m_messages[i];
	if (tmp.status == kApplied) { //kToBeDiscarded
		posElementsToBeRemoved.push_back(i);
	}
}

if (posElementsToBeRemoved.size()>0){
cout << " ApplicationLogic::CheckForRequiredSubscriptions() .erese "<< posElementsToBeRemoved.size() << " msg"<< endl;
for (unsigned i=0; i< posElementsToBeRemoved.size(); ++i){
	m_messages.erase (m_messages.begin()+posElementsToBeRemoved[i]);
}
}
